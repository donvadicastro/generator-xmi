...
describe('xmiParser', () => {
    describe('Activities', () => {
        it('Verify activity diagram', async () => {
            ...
            expect(parser.elements.map(x => x.name).filter(x => x)).toEqual(["sequence", "actor1", "c1", "c2"]);
    ...
    describe('Activities loop', () => {
        it('verify activity diagram with loop', async () => {
            const data = readJSONSync(path.resolve(__dirname, '../../../resources/models/project11_activity_loop.json'));
            const parser = new XmiParser(data);
            ...
            expect(parser.elements.filter(x => x).map(x => x.name).filter(x => x)).toEqual(["componentModel", "c1", "c2", "start"]);
            expect(sequence).toBeInstanceOf(xmiCollaboration);
